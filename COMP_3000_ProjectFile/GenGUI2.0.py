...
# Define a Base64 character set (no '=' padding character included)
BASE64_CHARS = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"

def fetch_random_seed():
    """Fetch a random seed from Random.org using Base64 characters only."""
...
def generate_random_string(length):
    """
    Generate a random string of given length using only Base64 characters.
    """
    return ''.join(random.choice(BASE64_CHARS) for _ in range(length))

def generate_otp_page(identifier_length=8, page_length=3500):
    """
    Generate a single OTP page with a random identifier + OTP content
    using only Base64 characters.
    """
...
def generate_otp_file(file_name="otp_cipher.txt", num_pages=10000, mode="standard"):
    """
    Generate an OTP file with each page on a new line, strictly using Base64 characters.
